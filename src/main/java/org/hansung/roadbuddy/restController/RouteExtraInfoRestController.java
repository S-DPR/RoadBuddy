...
public class RouteExtraInfoRestController extends GenericRestController {
    ...
    public ResponseEntity getRailTransfer(@RequestBody Legs legs) throws JsonProcessingException {
        List<List<RailTransferResDto<List<String>>>> ret = railAPIService.getSubwayTransfer(legs);
        List<List<SteepSlope>> steepSlopes = steepSlopeService.updateSteepSlopes(legs);
        return toResponse(steepSlopes);
    }
}
