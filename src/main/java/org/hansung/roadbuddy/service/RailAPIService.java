...
public class RailAPIService extends GenericAPIService {
    ...
    public List<List<RailTransferResDto<List<String>>>> getSubwayTransfer(Legs legs) throws JsonProcessingException {
        List<Steps> steps = legs.getSteps();
        int len = steps.size();
        List<List<RailTransferResDto<List<String>>>> transferResDtos = new ArrayList<>(len);
        for (int i = 0; i < len-2; i++) {
            ...
            transferResDtos.set(i + 1, path);
        }
        return transferResDtos;
    }
